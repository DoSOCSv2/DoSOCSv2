...
class packageInfo:
	...
	def __init__(self, packagePath, packageVersion = None, packageSupplier = None, packageOriginator = None, packageDownloadLocation = None, packageHomePage = None, packageSourceInfo = None, packageLicenseComments = None, packageDescription = None):
	...
	def outputPackageInfo_TAG(self):
		...
		if self.packageOriginator != None:
			output += "PackageOriginator: " + str(self.packageOriginator) + '\n'
			
		output += "PackageDownloadLocation:	" + str(self.packageDownloadLocation) + '\n'
		output += "PackageVerificationCode: " + str(self.packageVerificationCode) + '\n'
		...
		if self.packageSourceInfo != None:
			output += "PackageSourceInfo: " + str(self.packageSourceInfo) + '\n'
			
		output += "PackageLicenseConcluded: " + str(self.packageLicenseConcluded) + '\n'
		output += "PackageLicenseDeclared: " + str(self.packageLicenseDeclared) + '\n'
		...
		if self.packageLicenseComments != None:
			output += "PackageLicenseComments: <text>" + str(self.packageLicenseComments) + "</text>\n"
			
		output += "PackageCopyrightText: <text>" + str(self.packageCopyrightText) + "</text>\n"
		
		if self.packageSummary != None:
			output += "PackageSummary: <text>" + str(self.packageSummary) + "</text>\n"
		if self.packageDescription != None:
			output += "PackageDescription: <text>" + str(self.packageDescription) + "</text>\n"
	...
	def outputPackageInfo_RDF(self):
		...
		if self.packageVersion != None:
			output += '\t\t<versionInfo>' + str(self.packageVersion) + '</versionInfo>\n'
		if self.packageFileName != None:
			output += '\t\t<PackageFileName>' + str(self.packageFileName) + '</packageFileName>\n'
		if self.packageSupplier != None:
			output += '\t\t<supplier>' + str(self.packageSupplier) + '</supplier>\n'
		if self.packageOriginator != None:
			output += '\t\t<originator>' + str(self.packageOriginator) + '</originator>\n'
			
		output += '\t\t<downloadLocation>' + str(self.packageDownloadLocation) + '</downloadLocation>\n'
		output += '\t\t<packageVerificationCode>\n'
		output += '\t\t\t<PackageVerificationCode>\n'
		output += '\t\t\t<packageVerificationCodeValue>' + str(self.packageVerificationCode) + '</packageVerificationCodeValue>\n'
		output += '\t\t\t<packageVerificationCodeExcludedFile>' + str(self.packageVerificationCodeExcludedFile) +  '</packageVerificationCodeExcludedFile>\n'
		output += '\t\t\t</PackageVerificationCode>\n'
		output += '\t\t</packageVerificationCode>\n'
		output += '\t\t<checksum>\n'
		output += '\t\t\t<Checksum>\n'
		output += '\t\t\t\t<algorithm rdf:resource="' + str(self.packageChecksumAlgorithm) + '"/>\n'
		output += '\t\t\t\t<checksumValue>' + str(self.packageChecksum) + '</checksumValue>\n'
		output += '\t\t\t</Checksum>\n'
		output += '\t\t</checksum>\n'
		
		if self.packageHomePage != None:
			output += '\t\t<' + str(self.packageName) + ':homepage rdf:resource="' + str(self.packageHomePage) + '"/>\n'
		if self.packageSourceInfo != None:
			output += '\t\t<sourceInfo>' + str(self.packageSourceInfo) + '</sourceInfo>\n'
		if self.packageLicenseConcluded  != None:
			output += '\t\t<licenseConcluded>\n'
			output += '\t\t\t<DisjunctiveLicenseSet>\n'
			output += '\t\t\t\t<member rdf:resource="' + str(self.packageLicenseConcluded) + '"/>\n'
			output += '\t\t\t</DisjunctiveLicenseSet>\n'
			output += '\t\t</licenseConcluded>\n'
		if self.packageLicenseDeclared != None:
			output += '\t\t<licenseDeclared>\n'
			output += '\t\t\t<ConjunctiveLicenseSet>\n'
			output += '\t\t\t\t<member rdf:resource="' + str(self.packageLicenseDeclared) + '" />\n'
			output += '\t\t\t</ConjunctiveLicenseSet>\n'
			output += '\t\t</licenseDeclared>\n'
		
		for license in self.packageLicenseInfoFromFiles:
			output += '\t\t<licenseInfoFromFiles rdf:resource="' + str(license) + '" />\n'
			
		if self.packageLicenseComments != None:
			output += '\t\t<licenseComments>' + str(self.packageLicenseComments) + '</licenseComments>\n'
			
		output += '\t\t<copyrightText>' + str(self.packageCopyrightText) + '</copyrightText>\n'
		
		if self.packageSummary != None:
			output += '\t\t<summary>' + str(self.packageSummary) + '</summary>\n'
		if self.packageDescription != None:
			output += '\t\t<description>' + str(self.packageDescription) + '</description>\n'
